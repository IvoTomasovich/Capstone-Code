...
Two-stage transcript correction is applied to retrieved context before
the RAG model answers:
  Stage 1 — rule-based street name correction  (fuzzy matching)
  Stage 2 — rule-based person name correction  (fuzzy matching)

Both stages are fully deterministic — no extra LLM calls.
...
# ── Correction pipeline import (fuzzy-only, no LLM) ─────────────
...
class RAG(dspy.Module):
    ...
    def forward(self, question, start_date, end_date, n_results=5):
        ...
        # ─────────────────────────────────────────────────────────
        # Apply two-stage fuzzy transcript correction before the
        # RAG model sees the context.  No extra LLM calls — this
        # is pure deterministic fuzzy matching.
        # ─────────────────────────────────────────────────────────
        corrected_context, corrections = correct_transcript(
            context_str, correction_dicts
        )
        if corrections:
            print(f"[rag] Applied {len(corrections)} corrections to retrieved context")

        response = self.respond(context=corrected_context, question=question)
...
# -- Correction dictionaries (NEW) --------------------------------
#    Loaded once at startup.  No LLM needed — correction is pure
#    fuzzy matching against these dictionaries.
correction_dicts = load_dictionaries(
    english_path=os.path.join(os.path.dirname(__file__), "english_words.json"),
    names_path=os.path.join(os.path.dirname(__file__), "nola_names.json"),
    streets_path=os.path.join(os.path.dirname(__file__), "nola_streets.json"),
)
